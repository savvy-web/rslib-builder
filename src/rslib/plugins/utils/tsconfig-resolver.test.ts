...
import {
	TsconfigResolver,
	TsconfigResolverError,
	convertImportsNotUsedAsValues,
	convertJsxEmit,
	convertLibReference,
...
// Tests for backward-compatible function exports
describe("tsconfig-resolver functions (backward compatibility)", () => {
	describe("convertScriptTarget", () => {
		it("should convert ES2022 target", () => {
			expect(convertScriptTarget(ScriptTarget.ES2022)).toBe("es2022");
		});

		it("should convert ESNext target", () => {
			expect(convertScriptTarget(ScriptTarget.ESNext)).toBe("esnext");
		});

		it("should convert ES5 target", () => {
			expect(convertScriptTarget(ScriptTarget.ES5)).toBe("es5");
		});

		it("should convert Latest target to esnext", () => {
			expect(convertScriptTarget(ScriptTarget.Latest)).toBe("esnext");
		});

		it("should return undefined for undefined input", () => {
			expect(convertScriptTarget(undefined)).toBeUndefined();
		});

		it("should handle all ES versions", () => {
			expect(convertScriptTarget(ScriptTarget.ES2015)).toBe("es2015");
			expect(convertScriptTarget(ScriptTarget.ES2016)).toBe("es2016");
			expect(convertScriptTarget(ScriptTarget.ES2017)).toBe("es2017");
			expect(convertScriptTarget(ScriptTarget.ES2018)).toBe("es2018");
			expect(convertScriptTarget(ScriptTarget.ES2019)).toBe("es2019");
			expect(convertScriptTarget(ScriptTarget.ES2020)).toBe("es2020");
			expect(convertScriptTarget(ScriptTarget.ES2021)).toBe("es2021");
			expect(convertScriptTarget(ScriptTarget.ES2023)).toBe("es2023");
			expect(convertScriptTarget(ScriptTarget.ES2024)).toBe("es2024");
		});
	});

	describe("convertModuleKind", () => {
		it("should convert NodeNext module", () => {
			expect(convertModuleKind(ModuleKind.NodeNext)).toBe("nodenext");
		});

		it("should convert ESNext module", () => {
			expect(convertModuleKind(ModuleKind.ESNext)).toBe("esnext");
		});

		it("should convert CommonJS module", () => {
			expect(convertModuleKind(ModuleKind.CommonJS)).toBe("commonjs");
		});

		it("should convert Node16 module", () => {
			expect(convertModuleKind(ModuleKind.Node16)).toBe("node16");
		});

		it("should convert Preserve module", () => {
			expect(convertModuleKind(ModuleKind.Preserve)).toBe("preserve");
		});

		it("should return undefined for undefined input", () => {
			expect(convertModuleKind(undefined)).toBeUndefined();
		});

		it("should handle other module kinds", () => {
			expect(convertModuleKind(ModuleKind.AMD)).toBe("amd");
			expect(convertModuleKind(ModuleKind.UMD)).toBe("umd");
			expect(convertModuleKind(ModuleKind.System)).toBe("system");
			expect(convertModuleKind(ModuleKind.ES2015)).toBe("es2015");
			expect(convertModuleKind(ModuleKind.ES2020)).toBe("es2020");
			expect(convertModuleKind(ModuleKind.ES2022)).toBe("es2022");
			expect(convertModuleKind(ModuleKind.None)).toBe("none");
		});

		it("should convert Node18 module (enum value 101)", () => {
			expect(convertModuleKind(101 as ModuleKind)).toBe("node18");
		});

		it("should convert Node20 module (enum value 102)", () => {
			expect(convertModuleKind(102 as ModuleKind)).toBe("node20");
		});
	});

	describe("convertModuleResolution", () => {
		it("should convert NodeNext resolution", () => {
			expect(convertModuleResolution(ModuleResolutionKind.NodeNext)).toBe("nodenext");
		});

		it("should convert Bundler resolution", () => {
			expect(convertModuleResolution(ModuleResolutionKind.Bundler)).toBe("bundler");
		});

		it("should convert Node16 resolution", () => {
			expect(convertModuleResolution(ModuleResolutionKind.Node16)).toBe("node16");
		});

		it("should convert NodeJs resolution to node10 (NodeJs is deprecated alias for Node10)", () => {
			// NodeJs and Node10 share the same enum value (2)
			expect(convertModuleResolution(ModuleResolutionKind.NodeJs)).toBe("node10");
		});

		it("should convert Classic resolution", () => {
			expect(convertModuleResolution(ModuleResolutionKind.Classic)).toBe("classic");
		});

		it("should return undefined for undefined input", () => {
			expect(convertModuleResolution(undefined)).toBeUndefined();
		});
	});

	describe("convertJsxEmit", () => {
		it("should convert react-jsx", () => {
			expect(convertJsxEmit(JsxEmit.ReactJSX)).toBe("react-jsx");
		});

		it("should convert preserve", () => {
			expect(convertJsxEmit(JsxEmit.Preserve)).toBe("preserve");
		});

		it("should convert react", () => {
			expect(convertJsxEmit(JsxEmit.React)).toBe("react");
		});

		it("should convert react-native", () => {
			expect(convertJsxEmit(JsxEmit.ReactNative)).toBe("react-native");
		});

		it("should convert react-jsxdev", () => {
			expect(convertJsxEmit(JsxEmit.ReactJSXDev)).toBe("react-jsxdev");
		});

		it("should convert none", () => {
			expect(convertJsxEmit(JsxEmit.None)).toBe("none");
		});

		it("should return undefined for undefined input", () => {
			expect(convertJsxEmit(undefined)).toBeUndefined();
		});
	});

	describe("convertModuleDetection", () => {
		it("should convert auto", () => {
			expect(convertModuleDetection(ModuleDetectionKind.Auto)).toBe("auto");
		});

		it("should convert force", () => {
			expect(convertModuleDetection(ModuleDetectionKind.Force)).toBe("force");
		});

		it("should convert legacy", () => {
			expect(convertModuleDetection(ModuleDetectionKind.Legacy)).toBe("legacy");
		});

		it("should return undefined for undefined input", () => {
			expect(convertModuleDetection(undefined)).toBeUndefined();
		});
	});

	describe("convertNewLine", () => {
		it("should convert lf", () => {
			expect(convertNewLine(NewLineKind.LineFeed)).toBe("lf");
		});

		it("should convert crlf", () => {
			expect(convertNewLine(NewLineKind.CarriageReturnLineFeed)).toBe("crlf");
		});

		it("should return undefined for undefined input", () => {
			expect(convertNewLine(undefined)).toBeUndefined();
		});
	});

	describe("convertImportsNotUsedAsValues", () => {
		it("should convert remove", () => {
			expect(convertImportsNotUsedAsValues(ImportsNotUsedAsValues.Remove)).toBe("remove");
		});

		it("should convert preserve", () => {
			expect(convertImportsNotUsedAsValues(ImportsNotUsedAsValues.Preserve)).toBe("preserve");
		});

		it("should convert error", () => {
			expect(convertImportsNotUsedAsValues(ImportsNotUsedAsValues.Error)).toBe("error");
		});

		it("should return undefined for undefined input", () => {
			expect(convertImportsNotUsedAsValues(undefined)).toBeUndefined();
		});
	});

	describe("convertLibReference", () => {
		it("should convert lib.esnext.d.ts to esnext", () => {
			expect(convertLibReference("lib.esnext.d.ts")).toBe("esnext");
		});

		it("should convert lib.dom.d.ts to dom", () => {
			expect(convertLibReference("lib.dom.d.ts")).toBe("dom");
		});

		it("should convert lib.es2022.d.ts to es2022", () => {
			expect(convertLibReference("lib.es2022.d.ts")).toBe("es2022");
		});

		it("should handle full path with forward slashes", () => {
			expect(convertLibReference("/path/to/node_modules/typescript/lib/lib.dom.d.ts")).toBe("dom");
		});

		it("should handle full path with backslashes", () => {
			expect(convertLibReference("C:\\path\\to\\node_modules\\typescript\\lib\\lib.dom.d.ts")).toBe("dom");
		});

		it("should handle compound lib names", () => {
			expect(convertLibReference("lib.es2022.intl.d.ts")).toBe("es2022.intl");
			expect(convertLibReference("lib.esnext.array.d.ts")).toBe("esnext.array");
		});
	});

	describe("normalizePathToRelative", () => {
		it("should convert absolute unix path to relative", () => {
			const result = normalizePathToRelative("/project/src", "/project");
			expect(result).toBe("./src");
		});

		it.skipIf(process.platform !== "win32")("should convert absolute windows path to relative", () => {
			// This test only makes sense on Windows where C:\ paths are absolute
			const result = normalizePathToRelative("C:\\project\\src", "C:\\project");
			expect(result).toBe("./src");
		});

		it("should preserve already relative paths", () => {
			expect(normalizePathToRelative("./src", "/project")).toBe("./src");
			expect(normalizePathToRelative("../other", "/project")).toBe("../other");
		});

		it("should return undefined for undefined input", () => {
			expect(normalizePathToRelative(undefined, "/project")).toBeUndefined();
		});

		it("should handle nested directories", () => {
			const result = normalizePathToRelative("/project/src/components", "/project");
			expect(result).toBe("./src/components");
		});
	});

	describe("convertParsedConfigToJson", () => {
		it("should include $schema in output", () => {
			const parsed: ParsedCommandLine = {
				options: {},
				fileNames: [],
			...
			const result = convertParsedConfigToJson(parsed, "/project");
			expect(result.$schema).toBe("https://json.schemastore.org/tsconfig");
		});
