...
export class RSPressPluginBuilder {
	...
	private static async createRuntimeLib(
		...
		const sourceMap = mode === "dev";
		const baseOutputDir = `dist/${mode}/runtime`;

		// Build plugins list
		const plugins: RsbuildPlugin[] = [];

		// DtsPlugin for runtime types — no dtsPathPrefix needed because the runtime
		// lib already outputs to dist/<mode>/runtime/ via distPath.root.
