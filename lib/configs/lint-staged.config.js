...
export default {
	...
	// Lint TSDoc syntax and typecheck TypeScript files
	"*.{ts,cts,mts,tsx}":
		/** @param {string[]} filenames */
		(filenames) => {
			const srcFiles = filenames.filter((f) => f.includes("src/") && !f.includes(".test."));
			return [
				...(srcFiles.length > 0 ? [`eslint --config ./lib/configs/eslint.config.ts ${srcFiles.join(" ")}`] : []),
				filenames.length > 0 ? "pnpm exec tsgo --noEmit" : [],
			].flat();
		},
};
